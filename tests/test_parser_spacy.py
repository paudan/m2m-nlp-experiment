...
import spacy
import unittest
from parsing import process_parsed, leftmost_verb
...
class SpacyParserTestCase(unittest.TestCase):
    ...
    def test_case1(self):
        ...
        doc = self.nlp(phrase)
        self.assertSetEqual(process_parsed(doc), {('produce', 'test software'), ('Initiate', 'test software')})

    def test_case2(self):
        phrase = "Implement specification, software and tests"
        doc = self.nlp(phrase)
        self.assertSetEqual(process_parsed(doc),
    ...
    def test_leftmost(self):
        ...
        self.assertEquals(leftmost_verb(doc[9]).text, 'start')

    def test_leftmost_case2(self):
        phrase = "Implement specification, software and tests"
        doc = self.nlp(phrase)
        self.assertEquals(doc[5].text, 'tests')
        self.assertEquals(leftmost_verb(doc[5]).text, 'Implement')

    def test_leftmost_case3(self):
        doc = self.nlp("Implement initial specification, required software or test items")
        self.assertEquals(doc[8].text, 'items')
        self.assertEquals(leftmost_verb(doc[8]).text, 'Implement')
    ...
    def test_leftmost_case4(self):
        ...
        self.assertEquals(leftmost_verb(doc[7]).text, 'create')
        self.assertEquals(doc[4].text, 'software')
        self.assertEquals(leftmost_verb(doc[4]).text, 'produce')
