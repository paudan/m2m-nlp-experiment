import os
import unittest
from processing import BertNLPProcessor


class BertProcessingTestcase(unittest.TestCase):
    processor = None

    @classmethod
    def setUpClass(cls):
        os.chdir('..')
        cls.processor = BertNLPProcessor()
    ...
    def test_extract_individual_concepts(self):
        ...
        self.assertListEqual(concepts, [])  # Fails to find "America"

    def test_corner_case(self):
        verb = self.processor.extract_verb_phrase('Review & Escalate')
        self.assertEqual(verb, 'review')
